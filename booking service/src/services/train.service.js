...
async function searchTrains(source, destination) {
    ...
    const result = await pool.query(
        'SELECT DISTINCT t.id, t.train_number, t.train_name FROM trains t JOIN train_stations ts1 ON t.id = ts1.train_id JOIN train_stations ts2 ON t.id = ts2.train_id JOIN stations s1 ON ts1.station_id = s1.id JOIN stations s2 ON ts2.station_id = s2.id WHERE s1.station_code = $1 AND s2.station_code = $2 AND ts1.stop_order < ts2.stop_order',
        [source, destination]
    );
...
async function bookTicket(data) {
    ...
    const trainResponse = await axios.get(`${process.env.TRAIN_SERVICE_URL}/trains/${trainId}`);
...
if (!isLocked) {
    ...
    logger.warn(
        `Seat Locked By Another User | Seat:${seatNumber}`
    );
    ...
    logger.info(`Booking request received | User:${userId} Train:${trainId} Seat:${seatNumber}`);
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const seatResult = await client.query('SELECT * FROM seat_inventory WHERE train_id = $1 AND travel_date = $2 AND seat_number = $3 FOR UPDATE', [
            trainId,
            travelDate,
            seatNumber,
        ]);
        ...
        await client.query('INSERT INTO bookings ( user_id, train_id, travel_date, seat_number, booking_status ) VALUES ($1,$2,$3,$4,$5)', [
            userId,
            trainId,
            travelDate,
            seatNumber,
        ...
        await client.query('UPDATE seat_inventory SET status= \'BOOKED\' WHERE train_id=$1 AND travel_date=$2 AND seat_number=$3', [trainId, travelDate, seatNumber]);
        await client.query('COMMIT');
        logger.info(`Seat booked successfully | User:${userId} Seat:${seatNumber}`);
        getIO().emit('seatBooked', { trainId, travelDate, seatNumber });
...
async function getMyBookings(userId) {
    const result = await pool.query('SELECT b.id, t.train_name, b.seat_number, b.travel_date, b.booking_status FROM bookings b JOIN trains t ON b.train_id=t.id WHERE b.user_id=$1', [
        userId,
    ]);
    return result.rows;
}
...
async function cancelTicket(data) {
    ...
    try {
        ...
        await client.query('UPDATE seat_inventory SET status= \'AVAILABLE\' WHERE train_id=$1 AND travel_date=$2 AND seat_number=$3', [
            booking.train_id,
            booking.travel_date,
            booking.seat_number,
        ]);
...
async function getAvailableSeats(trainId, travelDate) {
    const result = await pool.query('SELECT * FROM seat_inventory WHERE train_id=$1 AND travel_date=$2 AND status= \'AVAILABLE\'', [trainId, travelDate]);
    return { availableSeats: result.rows.length, seats: result.rows };
}
